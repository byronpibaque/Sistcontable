...
export default {
    ...
    addAccesos: async(req,res,next)=>{
        try {
            ...
            const updatedObject= await models.Usuario_esquema
    ...
    updateDatosPersonales: async (req,res,next) => {
        try { 
            const reg = await models.Usuario_esquema.findByIdAndUpdate({_id:req.body._id},
    ...
    remove: async (req,res,next) => {
        try {
            const reg = await models.Usuario_esquema.findByIdAndDelete({_id:req.query._id});
            res.status(200).json(reg);
        } catch(e){
            res.status(500).send({
                message:'Ocurrió un error'
    ...
    activate: async (req,res,next) => {
        try {
            const reg = await models.Usuario_esquema.findByIdAndUpdate({_id:req.body._id},{estado:1});
            res.status(200).json(reg);
        } catch(e){
            res.status(500).send({
                message:'Ocurrió un error'
    ...
    deactivate:async (req,res,next) => {
        try {
            const reg = await models.Usuario_esquema.findByIdAndUpdate({_id:req.body._id},{estado:0});
            res.status(200).json(reg);
        } catch(e){
            res.status(500).send({
                message:'Ocurrió un error'
    ...
    login:async(req,res,next)=>{
        try {
            ...
            let match=""
            let user = await models.Usuario_esquema
            .findOne({$or:[{"login.usuario":req.query.data},{"correo":req.query.data}]});
            if (user) { 
